...
namespace Edwards.Scada.Test.Framework.GlobalHelper
{
    public class WindowAppServices
    {
        ...
        public void DeleteExistingDLL()
        {
            if (File.Exists(@"C:\Program Files (x86)\Edwards\DataLogger\TsStorePlugins\Edwards.TsStore.Plugin.Cumulocity.dll"))
            {
                File.Delete(@"C:\Program Files (x86)\Edwards\DataLogger\TsStorePlugins\Edwards.TsStore.Plugin.Cumulocity.dll");
            }
        }
        ...
        public void CloseSSMSWithoutSave()
        {
            ...
            // ssmsWindow.FindFirstDescendant(cf.ByName("No")).AsButton().Click();
        }
        ...
        public void CopyDLLFile()
        {

            string oldPath = @"C:\Program Files (x86)\Edwards\DataLogger\TsStorePlugins\Edwards.TsStore.Plugin.Cumulocity.dll";
            string newpath = @"C:\Program Files (x86)\Edwards\DataLogger\TsStorePlugins\";
            string newFileName = "Edwards.TsStore.Plugin.Cumulocity.dll.DISABLED";
        ...
        public void CopyTemplateDLLFile()
        {

            string oldPath = @"C:\Program Files (x86)\Edwards\DataLogger\TsStorePlugins\TsStorePluginTemplate.dll.DISABLED";
            string newpath = @"C:\Program Files (x86)\Edwards\DataLogger\TsStorePlugins\";
            string newFileName = "TsStorePluginTemplate.dll";
            FileInfo f1 = new FileInfo(oldPath);
            if (f1.Exists)
            {
                if (!Directory.Exists(newpath))
        ...
        {
            string filepath = @"C:\Program Files (x86)\Edwards\DataLogger\TsStorePlugins\Edwards.TsStore.Plugin.Cumulocity.dll.DISABLED";
            string filePathWithoutExt = Path.ChangeExtension(filepath, null);

            Console.WriteLine(filePathWithoutExt);
        }
        public void ChangeExtension(string sourceFileName)
        {
            string destfileName = Path.ChangeExtension(sourceFileName, ".dll.DISABLED");
          //  winApp.
           // winApp.Get(SearchCriteria.ByText(sourceFileName)).RightClick();
        ...
        {
            var services = System.IO.Path.Combine(Environment.SystemDirectory, "services.msc");
            serviceProcess = System.Diagnostics.Process.Start(services, "/name Services.Edwards Scada Agent Framework");
