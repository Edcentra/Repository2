...
And login user 'administrator' created new SMTP page From as 'Edwards.Support@edwardsvacuum.com' destination with SMTP IP of '160.100.30.222', port number as '25' and To address as 'xyz@atlascopco.com'  
And navigated to Dispatch Manager->Scheduler.
When Clicked Week days check box, All Day Check box and selected previously created PageDestination'Admin, EdCentra (administrator) (SMTP)'.
Then clicked Apply
When Go to Dispatch Manager->Inhibit Settings tab and Click on the [New] button
Then A new Inhibit form should be visible and delete already created alert inhibit'Admin, EdCentra (administrator)'
...
Then Inhibit Settings should be saved and new entry 'Admin, EdCentra (administrator)' entry should be added on the top list
