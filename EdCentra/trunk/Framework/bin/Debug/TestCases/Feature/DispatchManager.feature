...
Scenario: Dispatch Manager - Configure SMTP Paging
    ...
    And login user 'administrator' created new SMTP page From as 'Edwards.Support@edwardsvacuum.com' destination with SMTP IP of '160.100.30.222', port number as '25' and To address as 'shalu.gupta@edwardsvacuum.com'  
...
Scenario: Dispatch Manager - Configure Other Paging
   ...
   And login user 'administrator' created new SMTP Auth page From as 'Edwards.Support@edwardsvacuum.com' destination with SMTP IP of '160.100.30.222', port number as '25' and To address as 'shalu.gupta@edwardsvacuum.com'
...
When Clicked Week days check box, All Day Check box and selected previously created PageDestination'Admin, EdCentra (administrator) (SMTPAuth)'.
And clicked Apply
Then The changes made in scheduling entry should be displayed and new PageDestination'Admin, EdCentra (administrator) (SMTPAuth)' entry added in the list.
...
And Edited the previous schedule PageDestination'(SMTPAuth)' entry(Select the scheduler entry in the list to edit it) edit with  SMTP page From as 'Edwards.Support@edwardsvacuum.com' destination with SMTP IP of '160.100.30.222' and To address as 'shalu.gupta@external.atlascopco.com' and add an Alternative Destination'Admin, EdCentra (administrator) (SMTP)' with an After Minutes value of one
Then The scheduling PageDestination'Admin, EdCentra (administrator) (SMTP)' entry should be displayed with a blue pointer indicating that escalation has been established
...
Then The schedule'Admin, EdCentra (administrator) (SMTP)' entry should display with the Start and End columns set
When Created an alert'18' from a piece of equipment'4002'
...
And login user 'testuser' created new SMTP page From as 'Edwards.Support@edwardsvacuum.com' destination with SMTP IP of '160.100.30.222', port number as '25' and To address as 'shalu.gupta@edwardsvacuum.com'  
Then The dispatcher details should display along with [Test], [Apply] and [Delete] buttons
When Clicked Test button
Then A message 'A test message has been placed on the queue.' should be displayed. 
And message ' Message [Test message] placed on queue for'dispayed in Autopager console with fields set to Subject as 'Subject: Test message' From as'From: Edwards.Support@edwardsvacuum.com' To as 'To: shalu.gupta@edwardsvacuum.com', content as 'Test message. Test message.' 
...
And Clicked Week days check box, All Day Check box and selected previously created PageDestination'Admin, EdCentra (administrator) (SMTPAuth)'.
And clicked Apply
